...
class EfficientNetLite(nn.Module):
    def __init__(self, widthi_multiplier, depth_multiplier, num_classes, drop_connect_rate, dropout_rate, q = False):
...
def build_effnetliteQ(name, num_classes, q = False):
    width_coefficient, depth_coefficient, _, dropout_rate = efficientnet_lite_params[name]
    model = EfficientNetLite(width_coefficient, depth_coefficient, num_classes, 0.2, dropout_rate, q = q)
    return model


if __name__ == '__main__':
    model_name = 'efficientnet_lite0'
    model = build_effnetliteQ(model_name, 1000)
    model.eval()
